                        ...
                        <div class="col-md-12">
                            <div class="collapse" id="themserver">
                                <div class="card card-body">
                                    ...
                                    <form action="{{route('server.store')}}" method="POST" role="form">
                                        @csrf
                            ...
                            <table class="table table-striped" id="">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$dt->name_server}}</td>
                                        <td>{{$dt->thong_tin_them}}</td>
                                        <td>
                                            @if($dt->action == 1)
                                        ...
                                        <td style="width:200px">
                                            <button data-toggle="modal" data-target="#modaledit{{$dt->id}}" class="btn btn-outline-primary">sửa</button>
                                            <a href="{{route('server.destroy',$dt->id)}}" class="btn btn-outline-danger btndelete">xóa</a>
                                        </td>
                                    </tr>

                                    <!-- model edit server -->
                                    <div class="modal fade" id="modaledit{{$dt->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="exampleModalLongTitle">edit {{$dt->name_server}}</h5>
                                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                    </button>
                                                </div>
                                                <div class="modal-body">
                                                <form action="{{route('server.update',$dt->id)}}" method="post" role="form">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="form-group">
                                                        <label>Name server</label>
                                                        <input type="text" class="form-control" name="name_server" placeholder="name server" value="{{$dt->name_server}}">
                                                    </div>
                                                    <div class="form-group">
                                                        <label>Thông tin thêm</label>
                                                        <input type="text" class="form-control" name="thong_tin_them" placeholder="Thông tin thêm" value="{{$dt->thong_tin_them}}">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="exampleFormControlSelect1">Trạng thái</label>
                                                        <select class="form-control" name="action">
                                                        <option value="1" {{($dt->action == 1) ? 'selected':''}}>Bật lên</option>
                                                        <option value="2" {{($dt->action == 2) ? 'selected':''}}>Tắt đi</option>
                                                        </select>
                                                    </div>
